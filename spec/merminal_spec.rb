# frozen_string_literal: true

require "spec_helper"
require "open3"
require "tmpdir"

RSpec.describe Merminal do
  it "reproduces property runs from a seed" do
    observed = []
    property(runs: 3, seed: 42) { |rng, _| observed << rng.rand(100) }
    expected_rng = Random.new(Integer(ENV.fetch("SEED", 42)))
    expect(observed).to eq(Array.new(3) { expected_rng.rand(100) })
  end

  it "reports a snapshot mismatch with its cell and code points" do
    updating = ENV.delete("UPDATE_SNAPSHOTS")
    matcher = match_snapshot("scene_demo_unicode")
    expect(matcher.matches?("x")).to be(false)
    expect(matcher.failure_message).to match(/1:1: U\+[0-9A-F]{4} .* expected, U\+0078 x actual/)
  ensure
    ENV["UPDATE_SNAPSHOTS"] = updating if updating
  end

  it "measures CJK, combining marks and ambiguous characters in cells" do
    expect(Merminal::Text.width("abc")).to eq(3)
    expect(Merminal::Text.width("日本語abc")).to eq(9)
    expect(Merminal::Text.width("한글")).to eq(4)
    expect(Merminal::Text.width("ｶﾅ")).to eq(2)
    expect(Merminal::Text.width("ＡＢ")).to eq(4)
    expect(Merminal::Text.width("e\u0301")).to eq(1)
    expect(Merminal::Text.width("a\uFE0F")).to eq(1)
    expect(Merminal::Text.width("·", ambiguous_width: 2)).to eq(2)
    expect(Merminal::Text.wrap("日本語abc", 4)).to eq(%w[日本 語ab c])
  end

  it "covers every light box-drawing arm mask and sorted width ranges" do
    table = Merminal::Raster::BOX_DRAWING
    (1..15).each do |mask|
      key = 4.times.map { |index| mask[index] == 1 ? 1 : 0 }.join
      expect(table).to have_key(key)
    end
    %w[┼ ╋ ╬ ┿ ╪].each { |character| expect(table).to have_value(character) }
    %i[WIDE ZERO AMBIGUOUS].each do |name|
      ranges = Merminal::Text.const_get(name)
      expect(ranges.each_cons(2).all? { |a, b| a.last < b.first }).to be(true)
    end
  end

  it "keeps original source line numbers through frontmatter and comments" do
    source = Merminal::Source.parse("---\ntitle: Chart\nfoo: bar\n---\n%% comment\ngraph LR\nA-->B\n")
    expect(source.title).to eq("Chart")
    expect(source.line_map).to eq([6, 7])
    expect(source.diagnostics.first.severity).to eq(:info)
  end

  it "handles empty and malformed source directives without losing lines" do
    expect(Merminal::Source.parse("").lines).to be_empty
    source = Merminal::Source.parse("---\ntitle: Plan\n---\n%%{init: {'theme':'solarized'}}%%\n%% note\ngraph LR\nA-->B\n")
    expect([source.title, source.directives["theme"], source.line_map]).to eq(["Plan", "solarized", [6, 7]])
    invalid = Merminal::Source.parse("%%{init: broken}%%\ngraph LR\n")
    expect(invalid.diagnostics.map(&:severity)).to include(:warning)
    unclosed = Merminal::Source.parse("---\ntitle: Plan\n")
    expect(unclosed.diagnostics.map(&:message)).to include("unclosed frontmatter")
  end

  it "renders connected boxes in both charsets and all directions" do
    %w[TB BT LR RL].each do |direction|
      input = "graph #{direction}\nA[日本語]-->B[Done]"
      unicode = described_class.render(input)
      ascii = described_class.render(input, charset: :ascii)
      expect(unicode).to include("日本語", "Done")
      expect(ascii).to match(/\A[\x20-\x7e\n]*\z/)
      expect(ascii).to include("??????")
      expect(unicode).to eq(described_class.render(input))
    end
  end

  it "reports malformed statements and can reject them in strict mode" do
    input = "graph LR\nA[broken\nB-->C"
    document = described_class.parse(input)
    expect(document.diagnostics.map(&:severity)).to include(:error)
    expect(document.ast.nodes.map(&:id)).to include("B", "C")
    expect { described_class.parse(input, strict: true) }.to raise_error(Merminal::SyntaxError)
  end

  it "rasterizes a junction and wide text without losing alignment" do
    scene = Merminal::Scene
    items = [
      scene::Box.new(rect: scene::Rect.new(x: 0, y: 0, width: 8, height: 3), stroke: scene::LIGHT,
                     corners: :sharp, role: :node_border, layer: :node),
      scene::Polyline.new(points: [[3, 0], [3, 2]], stroke: scene::LIGHT, role: :edge, layer: :edge),
      scene::Text.new(x: 2, y: 1, string: "日", role: :node_text, layer: :label, emphasis: nil)
    ]
    grid = Merminal::Raster.rasterize(scene.new(width: 8, height: 3, items: items))
    expect(Merminal::Output.render(grid)).to include("┬", "日")
  end

  it "renders the hand-built Scene demo in both charsets" do
    demo = File.expand_path("../examples/scene_demo.rb", __dir__)
    { unicode: [], ascii: ["--ascii"] }.each do |charset, arguments|
      output, errors, status = Open3.capture3(RbConfig.ruby, demo, *arguments)
      expect(status.exitstatus).to eq(0)
      expect(errors).to be_empty
      expect(output).to match_snapshot("scene_demo_#{charset}")
    end
  end

  it "separates independent edge crossings in bridge mode" do
    scene = Merminal::Scene
    items = [
      scene::Polyline.new(points: [[0, 2], [4, 2]], stroke: scene::LIGHT, role: :edge, layer: :edge),
      scene::Polyline.new(points: [[2, 0], [2, 4]], stroke: scene::LIGHT, role: :edge, layer: :edge)
    ]
    picture = scene.new(width: 5, height: 5, items: items)
    expect(Merminal::Output.render(Merminal::Raster.rasterize(picture))).to include("╶─┼─╴")
    expect(Merminal::Output.render(Merminal::Raster.rasterize(picture, crossings: :bridge))).to include("╶─│─╴")
  end

  it "keeps insertion order for items on the same layer" do
    glyph = Merminal::Scene::Glyph
    picture = Merminal::Scene.new(width: 1, height: 1, items: [
      glyph.new(x: 0, y: 0, char: "A", role: :series_1, layer: :marker),
      glyph.new(x: 0, y: 0, char: "B", role: :series_2, layer: :marker)
    ])
    expect(Merminal::Output.render(Merminal::Raster.rasterize(picture))).to eq("B")
  end

  it "parses modern shapes and applies Mermaid colors" do
    source = "flowchart LR\nA@{ shape: diamond } --> B[/Result/]\nclassDef hot fill:#f9f,stroke:#333,color:#000\nclass A hot"
    document = described_class.parse(source)
    expect(document.ast.nodes.map(&:shape)).to eq(%i[decision parallelogram])
    previous_no_color = ENV.delete("NO_COLOR")
    previous_color = ENV["COLORTERM"]
    ENV["COLORTERM"] = "truecolor"
    expect(document.render(color: true)).to include("48;2;255;153;255", "38;2;51;51;51")
  ensure
    ENV["NO_COLOR"] = previous_no_color if previous_no_color
    ENV["COLORTERM"] = previous_color
  end

  it "keeps semicolons inside quoted labels" do
    document = described_class.parse('graph LR; A["x;y"]; A-->B')
    expect(document.ast.nodes.map(&:label)).to eq(["x;y", "B"])
    expect(document.ast.edges.length).to eq(1)
    expect(document.diagnostics).to be_empty
  end

  it "keeps closing brackets inside quoted labels" do
    document = described_class.parse('graph LR; A["x]y;z"] --> B')
    expect(document.ast.nodes.map(&:label)).to eq(["x]y;z", "B"])
    expect(document.diagnostics).to be_empty
    slanted = described_class.parse('graph LR; A[/"x]y"/] --> B')
    expect(slanted.ast.nodes.first.label).to eq("x]y")
  end

  it "gives converging arrows separate input cells" do
    scene = described_class.parse("flowchart TB\nA-->C\nB-->C").scene
    arrowheads = scene.items.grep(Merminal::Scene::Marker).select { |item| item.kind == :arrow }
    expect(arrowheads.map { |item| [item.x, item.y] }.uniq.length).to eq(2)
  end

  it "shares channel tracks for disjoint edges and branches" do
    edges = described_class.parse("graph TB\nA-->B\nC-->D").scene.items.grep(Merminal::Scene::Polyline)
                           .select { |item| item.role == :edge }
    expect(edges.map { |edge| edge.points.length }).to eq([2, 2])
    edges = described_class.parse("graph TB\nA-->B\nA-->C").scene.items.grep(Merminal::Scene::Polyline)
                           .select { |item| item.role == :edge }
    expect(edges.first.points.first).to eq(edges.last.points.first)
    edges = described_class.parse("graph TB\nA-->C\nA-->D\nB-->C\nB-->D").scene.items
                           .grep(Merminal::Scene::Polyline).select { |item| item.role == :edge }
    expect(edges.first(2).map { |edge| edge.points[1][1] }.uniq.length).to eq(1)
    expect(edges.last(2).map { |edge| edge.points[1][1] }.uniq.length).to eq(1)
    expect(edges.first.points[1][1]).not_to eq(edges.last.points[1][1])
  end

  it "reduces crossings without making the initial order worse" do
    document = described_class.parse("graph TB\nA\nB\nC\nD\nA-->D\nB-->C")
    layout = Merminal::Flowchart::Layout.new(document.ast)
    layout.send(:orient_edges)
    layout.send(:assign_ranks)
    layout.send(:normalize_edges)
    ranks = layout.instance_variable_get(:@ranks)
    layout.instance_variable_set(:@groups, layout.instance_variable_get(:@work_nodes).group_by { |node| ranks.fetch(node.id) })
    expect(layout.send(:crossing_count)).to eq(1)
    layout.send(:order_nodes)
    expect(layout.send(:crossing_count)).to eq(0)
  end

  it "normalizes long and labeled edges before ordering" do
    source = "graph TB\nA ----> B\nB -->|a label| C"
    layout = Merminal::Flowchart::Layout.new(described_class.parse(source).ast)
    layout.scene
    paths = layout.instance_variable_get(:@edge_paths)
    ranks = layout.instance_variable_get(:@ranks)
    work = layout.instance_variable_get(:@work_nodes)
    positions = layout.instance_variable_get(:@positions)
    expect(paths.values.map(&:length)).to eq([4, 3])
    paths.each_value do |path|
      expect(path.each_cons(2).all? { |a, b| ranks[b] == ranks[a] + 1 }).to be(true)
    end
    dummies = work.grep(Merminal::Flowchart::Layout::WorkNode).select { |node| node.kind == :dummy }
    expect(dummies.map { |node| positions.fetch(node.id)[0] }.uniq.length).to eq(1)
    expect(work.grep(Merminal::Flowchart::Layout::WorkNode).count { |node| node.kind == :label }).to eq(1)
  end

  it "keeps ranked work nodes separated while aligning long edges" do
    property(runs: 100, seed: 407) do |rng, record|
      count = rng.rand(2..10)
      source = "graph #{%w[TB LR].sample(random: rng)}\n" +
               Array.new(count) { |index| "N#{index}" }.join("\n") + "\n" +
               Array.new(rng.rand(1..16)) do
                 "N#{rng.rand(count)} #{rng.rand(3).zero? ? '---->' : '-->'} N#{rng.rand(count)}"
               end.join("\n")
      record.call(source)
      gap = rng.rand(1..4)
      layout = Merminal::Flowchart::Layout.new(described_class.parse(source).ast, node_gap: gap)
      layout.scene
      horizontal = layout.instance_variable_get(:@horizontal)
      positions = layout.instance_variable_get(:@positions)
      sizes = layout.instance_variable_get(:@sizes)
      layout.instance_variable_get(:@groups).each_value do |nodes|
        nodes.each_cons(2) do |a, b|
          axis = horizontal ? 1 : 0
          expect(positions.fetch(b.id)[axis] - positions.fetch(a.id)[axis]).to be >= sizes.fetch(a.id)[axis] + gap
        end
      end
    end
  end

  it "reserves disjoint spaces for edge labels" do
    source = "graph TB\nA -->|first-long-label| B\nC -->|second-long-label| D"
    labels = described_class.parse(source).scene.items.grep(Merminal::Scene::Text)
                            .select { |item| item.role == :edge_label }
    expect(labels.length).to eq(2)
    expect(labels.first.y != labels.last.y ||
           labels.first.x + Merminal::Text.width(labels.first.string) <= labels.last.x ||
           labels.last.x + Merminal::Text.width(labels.last.string) <= labels.first.x).to be(true)
    expect(described_class.render(source)).to include("first-long-label", "second-long-label")
  end

  it "fits the public render API without truncating content" do
    source = "graph TB\nA[Hello] --> B[World]\nA --> C[Other]"
    compact = described_class.render(source, width: 24)
    expect(compact.lines.map { |line| Merminal::Text.width(line.chomp) }.max).to be <= 24
    expect(compact).to include("Hello", "World", "Other")
  end

  it "extracts only matching Markdown fences" do
    markdown = "````ruby\n```mermaid\ngraph LR\nA-->B\n````\n~~~mermaid\ngraph LR\nB-->C\n~~~\n"
    expect(described_class.markdown_blocks(markdown).map(&:line)).to eq([7])
  end

  it "runs the CLI for a pipe and reports check errors" do
    exe = File.expand_path("../exe/merminal", __dir__)
    output, errors, status = Open3.capture3(RbConfig.ruby, exe, "--ascii", stdin_data: "graph LR\nA-->B\n")
    expect(status.exitstatus).to eq(0)
    expect(errors).to be_empty
    expect(output).to include("A", "B", ">")
    _, errors, status = Open3.capture3(RbConfig.ruby, exe, "--check", stdin_data: "graph LR\nA[bad\n")
    expect(status.exitstatus).to eq(1)
    expect(errors).to include("unclosed node shape")
  end

  it "handles CLI files, Markdown, output, color, and exit codes" do
    exe = File.expand_path("../exe/merminal", __dir__)
    Dir.mktmpdir do |directory|
      markdown = File.join(directory, "diagrams.md")
      output_path = File.join(directory, "picture.txt")
      File.write(markdown, "# Example\n```mermaid\ngraph LR\nA-->B\n```\n")
      output, errors, status = Open3.capture3(RbConfig.ruby, exe, "--ascii", "-o", output_path, markdown)
      expect([status.exitstatus, output, errors]).to eq([0, "", ""])
      expect(File.read(output_path)).to include("A", "B", ">")

      output, errors, status = Open3.capture3({ "NO_COLOR" => nil, "FORCE_COLOR" => "1" },
                                              RbConfig.ruby, exe, "--color", "always", markdown)
      expect([status.exitstatus, errors]).to eq([0, ""])
      expect(output).to include("\e[")
    end
    _, _, status = Open3.capture3(RbConfig.ruby, exe, "--strict", stdin_data: "graph LR\nA[bad\n")
    expect(status.exitstatus).to eq(1)
    _, _, status = Open3.capture3(RbConfig.ruby, exe, stdin_data: "unknownDiagram\n")
    expect(status.exitstatus).to eq(3)
    _, _, status = Open3.capture3(RbConfig.ruby, exe, "--width", "0", stdin_data: "graph LR\nA-->B\n")
    expect(status.exitstatus).to eq(2)
  end

  it "renders every registered diagram with Unicode and ASCII" do
    samples = {
      sequence: "sequenceDiagram\nparticipant A as Alice\nparticipant B as Bob\nA->>B: Hello\nB-->>A: Hi\n",
      state: "stateDiagram-v2\n[*] --> Idle\nIdle --> Busy : work\nBusy --> [*]\n",
      class: "classDiagram\nclass Animal {\n+name: String\n+speak()\n}\nAnimal <|-- Dog\n",
      er: "erDiagram\nCUSTOMER ||--o{ ORDER : places\nCUSTOMER {\nstring name PK\n}\n",
      pie: "pie showData\ntitle Sales\n\"A\" : 30\n\"B\" : 70\n",
      xychart: "xychart-beta\ntitle Revenue\nx-axis [Jan, Feb, Mar]\nbar [2, 4, 3]\nline [1, 3, 5]\n",
      gantt: "gantt\ndateFormat YYYY-MM-DD\nsection Work\nPlan :done, p1, 2026-01-01, 3d\nBuild :active, b1, after p1, 5d\n",
      timeline: "timeline\ntitle History\n2024 : Alpha\n2025 : Beta\n",
      mindmap: "mindmap\nroot((Root))\n  A\n    A1\n  B\n"
    }
    expect(described_class.diagram_types).to include(:flowchart, *samples.keys)
    samples.each_value do |source|
      document = described_class.parse(source)
      expect(document.diagnostics.select { |diagnostic| diagnostic.severity == :error }).to be_empty
      expect(document.render).not_to be_empty
      expect(document.render(charset: :ascii)).to match(/\A[\x20-\x7e\n]*\z/)
    end
  end

  it "accepts the additional Mermaid syntax diagram declarations" do
    samples = {
      "journey" => "journey\ntitle Work\nTask: 5: Me\n",
      "quadrantChart" => "quadrantChart\ntitle Reach\nx-axis Low --> High\ny-axis Low --> High\nPoint: [0.4, 0.6]\n",
      "requirementDiagram" => "requirementDiagram\nrequirement api {\nid: 1\ntext: Public API\n}",
      "usecaseDiagram" => "usecaseDiagram\nactor User\nUser --> (Use system)\n",
      "usecase-beta" => "usecase-beta\nactor User\nUser --> (Use system)\n",
      "gitGraph" => "gitGraph\ncommit\nbranch feature\ncheckout feature\ncommit\n",
      "C4Context" => "C4Context\nPerson(user, \"User\")\nSystem(app, \"App\")\nRel(user, app, \"uses\")\n",
      "zenuml" => "zenuml\nAlice -> Bob: Hello\n",
      "sankey-beta" => "sankey-beta\nA,B,10\n",
      "block-beta" => "block-beta\nA:4\nB:4\nA --> B\n",
      "packet-beta" => "packet-beta\n0-7: \"Header\"\n",
      "kanban" => "kanban\nTodo\n  [Write docs]\n",
      "architecture-beta" => "architecture-beta\ngroup api\nservice web\n",
      "radar-beta" => "radar-beta\ntitle Skills\naxis A, B\ncurve C{1, 2}\n",
      "treemap-beta" => "treemap-beta\nbranch trunk\n",
      "treeView-beta" => "treeView-beta\nproject/\n  README.md\n",
      "cynefin-beta" => "cynefin-beta\ncomplex\n\"Unknown\"\n",
      "agentflow-beta" => "agentflow-beta TB\nflow reviewer[\"Review Agent\"]\n  changes[\"Gather changes\"]@{ shape: input }\n  analyse[\"Analyse\"]@{ shape: task }\n  changes --> analyse --> lint\nend\n",
      "railroad-ebnf-beta" => "railroad-ebnf-beta\nletter = \"a\" | \"b\" ;\n",
      "railroad-abnf-beta" => "railroad-abnf-beta\nletter = %x61 / %x62 ;\n",
      "railroad-peg-beta" => "railroad-peg-beta\nletter <- \"a\" / \"b\" ;\ndigit <- \"0\" ;\n",
      "railroad-beta" => "railroad-beta\nexpr = choice(terminal(\"a\"), nonterminal(\"letter\")) ;\n"
    }
    samples.each do |keyword, source|
      document = described_class.parse(source)
      expect(document.type).to eq(keyword.to_s.gsub(/[^a-zA-Z0-9]+/, "_").downcase.to_sym)
      expect(document.render).not_to be_empty
      expect(document.render(charset: :ascii)).to match(/\A[\x20-\x7e\n]*\z/)
    end
    expect(described_class.parse("agentflow\nA --> B").type).to eq(:agentflow_beta)
    agentflow = described_class.parse(samples.fetch("agentflow-beta"))
    expect([agentflow.ast.direction, agentflow.ast.edges.map { |edge| [edge.from, edge.to] }]).to eq([:TB, [["changes", "analyse"], ["analyse", "lint"]]])
    zenuml = described_class.parse(samples.fetch("zenuml"))
    expect(zenuml.ast.edges.map { |edge| [edge.from, edge.to, edge.label] }).to eq([["Alice", "Bob", "Hello"]])
    expect(described_class.render("zenuml\nA\nB\nA.SyncMessage(with, parameters)\nB.nestedSyncMessage()\n")).to include("SyncMessage", "nestedSyncMessage")
    zenuml_controls = described_class.render("zenuml\nAlice -> Bob: Start\nwhile(ok) {\n  Bob -> Alice: Retry\n}\nif(done) {\n  Alice -> Bob: Done\n} else {\n  Alice -> Bob: Failed\n}\n")
    expect(zenuml_controls).to include("while(ok)", "Retry", "if(done)", "else", "Failed")
    architecture = described_class.parse("architecture-beta LR\ngroup api\nservice web[Web]\nservice db[(DB)]\ndb:R --> L:web\n")
    expect([architecture.ast.direction, architecture.ast.edges.map { |edge| [edge.from, edge.to] }]).to eq([:LR, [["db", "web"]]])
    block = described_class.parse("block\ncolumns 3\na space:2 b\n")
    expect(block.type).to eq(:block_beta)
    block_render = described_class.render("block\ncolumns 3\na space:2 b\na-->b\n")
    expect(block_render).to include("a", "b", "a -> b")
    expect(block_render).not_to include("space:2", "a-->b")
    expect(described_class.parse("packet\n+8: \"Header\"\n").render).to include("Header", "0-7")
    %w[railroad-ebnf-beta railroad-abnf-beta railroad-peg-beta railroad-beta].each do |keyword|
      diagram = described_class.render(samples.fetch(keyword))
      expect(diagram).to include("letter"), keyword
      expect(diagram).to include(keyword == "railroad-abnf-beta" ? "%x61" : "a"), keyword
    end
    expect(described_class.render(samples.fetch("railroad-peg-beta"))).to include("digit")
    expect(described_class.render(samples.fetch("railroad-beta"))).to include("letter", "a")
    expect(described_class.render("railroad-ebnf-beta\nrule = \"Hello world\" /* comment */ ;\n")).to include("Hello world")
    expect(described_class.render("railroad-peg-beta\nrule <- !Keyword . ;\n")).to include("!", ".")
    expect(described_class.render("railroad-abnf-beta\na = \"a\" ; comment\nb = \"b\" ;\n")).to include("a", "b")
    kanban = described_class.render("kanban\n  Todo\n    [Create Documentation]\n  [In progress]\n    id6[Create renderer]\n")
    expect(kanban).to include("Todo", "Create Documentation", "In progress", "Create renderer")
    usecase = described_class.parse("usecase-beta\nactor Admin\nactor Person\nAdmin --|> Person\n")
    expect(usecase.ast.edges.map { |edge| [edge.from, edge.to] }).to eq([["Admin", "Person"]])
    anonymous_usecase = described_class.parse("usecase-beta\nactor User\nUser --> (Use system)\n")
    expect(anonymous_usecase.ast.edges.map { |edge| [edge.from, edge.to] }).to eq([["User", "Use_system"]])
    expect(described_class.render("usecase-beta\nactor User\nUser --> (Use system)\n")).to include("Use system")
    c4 = described_class.parse("C4Context\nPerson_Ext(user, \"User\")\nSystemDb(db, \"Database\")\nEnterprise_Boundary(boundary, \"Boundary\") {\n}\nRelIndex(1, user, db, \"uses\")\nUpdateLayoutConfig(4, 2)\n")
    expect(c4.ast.nodes.map { |node| [node.id, node.label] }).to include(["user", "User"], ["db", "Database"], ["boundary", "Boundary"])
    expect(c4.ast.edges.map { |edge| [edge.from, edge.to, edge.label] }).to eq([["user", "db", "uses"]])
    c4_variants = described_class.parse("C4Context\nPerson(customer, Customer, \"desc\")\nSystemQueue(queue, \"Queue\")\nRel(customer, queue, Uses)\n")
    expect(c4_variants.ast.nodes.map { |node| [node.id, node.label] }).to include(["customer", "Customer"], ["queue", "Queue"])
    expect(c4_variants.ast.edges.map { |edge| [edge.from, edge.to, edge.label] }).to eq([["customer", "queue", "Uses"]])
    requirement = described_class.parse("requirementDiagram\nfunctionalRequirement api {\nid: 1\n}\nelement service {\n}\napi - satisfies -> service\n")
    expect(requirement.ast.nodes.map(&:id)).not_to include("line1")
    expect(requirement.ast.edges.map { |edge| [edge.from, edge.to, edge.label] }).to eq([["api", "service", "satisfies"]])
    quadrant = described_class.parse(samples.fetch("quadrantChart"))
    expect(quadrant.ast.edges).to be_empty
    usecase_label = described_class.parse("usecase-beta\nactor User(\"Main user\")\nLogin[Sign in]\nUser --> Login\n")
    expect(usecase_label.ast.nodes.map { |node| [node.id, node.label] }).to include(["User", "Main user"], ["Login", "Sign in"])
      native = {
      journey: "journey\nsection Work\nTask: 5: Me\n",
      quadrant: "quadrantChart\nx-axis Low --> High\ny-axis Low --> High\nPoint: [0.4, 0.6]\n",
      sankey: "sankey-beta\nA,B,10\n",
      packet: "packet-beta\n0-3: \"Header\"\n",
      kanban: "kanban\ntodo[Todo]\n  task[Write docs]\n",
      radar: "radar-beta\naxis A[\"Alpha\"], B\naxis C[\"Gamma\"]\ncurve c{1,2,3}\n",
      treemap: "treemap-beta\n\"Root\"\n  \"Leaf\": 3\n",
      venn: "venn-beta\nset A: 10\nset B[\"Beta\"]: 20\nunion A,B[\"AB\"]: 5\ntext A1[\"Only A\"]\n",
      wardley: "wardley-beta\nanchor Users [0.9, 0.1]\ncomponent API [0.7, 0.4]\n",
      cynefin: "cynefin-beta\ncomplex\n\"Investigate\"\nclear\n\"Known path\"\ncomplex --> clear : \"Pattern\"\n",
      event_modeling: "eventModeling\ntf 01 ui CartUI\ntf 02 evt ItemAdded\n",
      swimlane: "swimlane-beta\nsubgraph Support[Support]\nA[Open] --> B[Close]\nend\n"
    }
    native.each do |name, source|
      rendered = described_class.render(source, charset: :ascii)
      expect(rendered).not_to be_empty, name.to_s
    end
    expect(described_class.render(native.fetch(:quadrant))).to include("Point")
    expect(described_class.render(native.fetch(:packet))).to include("Header", "0-3")
    expect(described_class.render(native.fetch(:kanban))).to include("Todo", "Write docs")
    expect(described_class.render("kanban\ntodo[Todo]\n  task[Write docs]@{ assigned: \"A\", priority: \"High\" }\n")).to include("assigned", "High")
    expect(described_class.render(native.fetch(:cynefin))).to include("Investigate", "Known path", "Pattern")
    expect(described_class.render(native.fetch(:event_modeling))).to include("CartUI", "ItemAdded")
    ishikawa = described_class.render("ishikawa-beta\nIncident\nPeople\n  Training\nProcess\n")
    expect(ishikawa).to include("Incident", "People", "Training", "Process")
    expect(described_class.render("eventmodeling\ntf 01 rmo CartUI ->> 02 ->> 03\nrf 02 evt CartCreated\nrf 03 evt ItemAdded\ndata ItemAdded01 {\n  description: string\n}\n")).to include("CartUI ->> 02", "02 ->> 03", "rf 02", "ItemAdded01")
    event_modeling_data = described_class.render("eventmodeling\ntf 01 ui CartUI\ntf 02 cmd AddItem { description: string }\ntf 03 evt Inventory.InventoryChanged [[InventoryChanged01]]\ndata InventoryChanged01 {\n  description: 'john'\n  price: 20.4\n}\n", charset: :ascii)
    expect(event_modeling_data).to include("AddItem { description: string }", "evt:Inventory", "InventoryChanged01", "price: 20.4")
    expect(event_modeling_data.scan("| pcr").length).to eq(1)
    aliases = described_class.render("eventmodeling\ntf 01 pcr First\ntimeframe 02 processor Second\n", charset: :ascii)
    expect(aliases.scan("| pcr").length).to eq(1)
    expect(described_class.render("eventModeling\nrf 01 evt External.InventoryChanged\n")).to include("External.InventoryChanged")
    expect(described_class.render("agentflow-beta\nflow reviewer[\"Review Agent\"]\nend\n")).to include("Review Agent")
    wardley = described_class.render("wardley-beta\nanchor Users [0.9, 0.1]\ncomponent API [0.7, 0.4]\ncomponent DB [0.4, 0.2]\nevolve API 0.85\nAPI +'uses'> Users\nAPI +< DB; reverse\nUsers -.-> API\nnote \"Key insight\" [0.4, 0.5]\nannotation 1,[0.5,0.5] \"Critical\"\nAPI -.- [0.8, 0.8]\n")
    expect(wardley).to include("Key insight", "uses", "reverse", "Critical", "trend")
    wardley_extensions = described_class.render("wardley-beta\npipeline Platform {\ncomponent API [0.7, 0.4] (buy)\ncomponent DB [0.4, 0.2] (inertia)\n}\nevolution Genesis -> Product -> Commodity\n")
    expect(wardley_extensions).to include("Pipelines", "Platform: API -> DB", "Evolution: Genesis -> Product -> Commodity", "(buy)", "(inertia)")
    expect(described_class.render(native.fetch(:swimlane))).to include("Support", "Open", "Close")
    tree = "treeView\nroot/\n├── lib/\n│   └── app.rb\n└── README.md\n"
    expect(described_class.render(tree)).to include("app.rb", "README.md")
    official = {
      usecase: "usecase-beta\nactor User as \"Main user\"\nLogin(\"Sign in\")\nUser --> Login\n",
      c4_boundary: "C4Container\nContainer_Boundary(payments, \"Payments\") {\nContainer(api, \"API\")\n}\n",
      gitgraph: "gitGraph TB:\ncommit id: \"root\"\ncommit id: \"release\" type: HIGHLIGHT tag: \"v1\"\n",
      quadrant: "quadrantChart\nquadrant-1 Strategic\nPoint: [0.8, 0.8]\n",
      radar: "radar-beta\naxis ruby[\"Ruby\"], rust[\"Rust\"]\ncurve team[\"Team\"]{ruby: 3, rust: 5}\n",
      treemap: "treemap-beta\n\"Section\"\n  \"Leaf\": 12\n",
      venn: "venn-beta\nset A\nset B[\"Users\"]\nunion A,B[\"Shared\"]\ntext A1[\"Only A\"]\n",
      cynefin: "cynefin-beta\nclear\n\"Known\"\ncomplex\n\"Probe\"\n"
    }
    expect(described_class.render(official.fetch(:usecase))).to include("Main user", "Sign in")
    expect(described_class.render(official.fetch(:c4_boundary))).to include("Payments", "API")
    expect(described_class.render(official.fetch(:gitgraph))).to include("root", "release", "v1")
    expect(described_class.render(official.fetch(:quadrant))).to include("Strategic", "Point")
    expect(described_class.render(official.fetch(:radar))).to include("Ruby", "Rust", "Team")
    expect { described_class.render("radar-beta\naxis A, B\ncurve c{1, malformed: 2}\n") }.not_to raise_error
    expect(described_class.render(official.fetch(:treemap))).to include("Section", "Leaf", "12")
    expect(described_class.render(official.fetch(:venn))).to include("Users", "Shared", "Only A")
    expect(described_class.render("venn-beta\ntitle \"Team overlap\"\nset A\nset B\n")).to include("Team overlap")
    expect(described_class.render("venn-beta\ntitle \"Team overlap\"\nset A\nset B\n")).not_to include("\"Team overlap\"")
    expect(described_class.render(official.fetch(:cynefin))).to include("Complex", "Clear", "Probe", "Known")
    Merminal::Diagrams::Additional::KEYWORDS.each do |keyword|
      document = described_class.parse("#{keyword}\nA --> B\n")
      expect(document.render(charset: :ascii)).to match(/\A[\x20-\x7e\n]*\z/)
    end
  end

  it "registers a diagram plugin and rejects duplicate keywords" do
    plugin = Module.new do
      def self.diagram_type = :sample
      def self.keywords = %w[sampleDiagram]
      def self.parse(_source) = [nil, []]
      def self.layout(_ast, **) = Merminal::Scene.new(width: 1, height: 1,
                                                         items: [Merminal::Scene::Glyph.new(x: 0, y: 0, char: "X",
                                                                                               role: :node_text, layer: :label)])
    end
    registry = Merminal.instance_variable_get(:@plugins)
    described_class.register(plugin)
    expect(described_class.render("sampleDiagram")).to eq("X")
    expect { described_class.register(plugin) }.to raise_error(ArgumentError, /duplicate/)
  ensure
    registry&.delete(plugin)
  end

  it "returns shareable parsed graphs and Scenes" do
    skip "Ractor unavailable" unless defined?(Ractor)

    document = described_class.parse("graph LR\nA-->B")
    expect(Ractor.shareable?(document.ast)).to be(true)
    expect(Ractor.shareable?(document.scene)).to be(true)
  end

  it "keeps wide first sequence participants inside the Scene" do
    source = "sequenceDiagram\nparticipant A as A very long first participant\nA->>B: Hello\n"
    expect { described_class.render(source) }.not_to raise_error
  end

  it "can repeat sequence participant boxes at the bottom" do
    source = "sequenceDiagram\nparticipant A as Alice\nparticipant B as Bob\nA->>B: Hello\n"
    output = described_class.render(source, repeat_participants: true)
    expect(output.scan("Alice").length).to eq(2)
    expect(output.scan("Bob").length).to eq(2)
  end

  it "keeps randomly generated flowcharts inside their Scenes" do
    property(runs: 500, seed: 20_260_921) do |rng, record|
      count = rng.rand(1..12)
      edges = Array.new(rng.rand(0..20)) do
        from, to = rng.rand(count), rng.rand(count)
        rng.rand(4).zero? ? "N#{from} -->|yes| N#{to}" : "N#{from} --> N#{to}"
      end
      source = "graph #{%w[TB BT LR RL].sample(random: rng)}\n" + edges.join("\n")
      record.call(source)
      assert_flowchart_properties(source)
    end
  end

  it "keeps every flowchart fixture inside its Scene" do
    paths = Dir.glob(File.expand_path("fixtures/{flowchart*,subgraph*}/**/*.mmd", __dir__))
    paths << File.expand_path("fixtures/flowchart.mmd", __dir__)
    expect(paths.length).to be >= 40
    paths.each { |path| assert_flowchart_properties(File.read(path)) }
  end

  it "contains nested subgraphs without crossing their sibling frames" do
    Dir.glob(File.expand_path("fixtures/subgraph_cases/*.mmd", __dir__)).each do |path|
      document = described_class.parse(File.read(path))
      groups = document.ast.subgraphs
      layout = Merminal::Flowchart::Layout.new(document.ast)
      layout.send(:orient_edges)
      layout.send(:assign_ranks)
      layout.send(:normalize_edges)
      layout.send(:order_nodes)
      layout.instance_variable_get(:@groups).each_value do |rank_nodes|
        groups.each do |group|
          positions = rank_nodes.each_index.select { |index| group.node_ids.include?(rank_nodes[index].id) }
          expect(positions.max - positions.min + 1).to eq(positions.length) unless positions.empty?
        end
      end
      frames = document.scene.items.grep(Merminal::Scene::Box).select { |item| item.role == :container_border }.map(&:rect)
      nodes = document.ast.nodes.map(&:id).zip(document.scene.items.grep(Merminal::Scene::Box)
                                  .select { |item| item.role == :node_border }.map(&:rect)).to_h
      expect(frames.length).to eq(groups.length)
      groups.zip(frames).each do |group, frame|
        group.node_ids.uniq.each do |id|
          node = nodes.fetch(id)
          expect(frame.x <= node.x && frame.y <= node.y &&
                 frame.x + frame.width >= node.x + node.width &&
                 frame.y + frame.height >= node.y + node.height).to be(true)
        end
        nodes.each do |id, node|
          next if group.node_ids.include?(id)

          expect(node.x + node.width <= frame.x || node.x >= frame.x + frame.width ||
                 node.y + node.height <= frame.y || node.y >= frame.y + frame.height).to be(true)
        end
      end
      edges = document.scene.items.grep(Merminal::Scene::Polyline).select { |item| item.role == :edge }
      document.ast.edges.zip(edges).each do |edge, line|
        groups.zip(frames).each do |group, frame|
          next if group.node_ids.include?(edge.from) || group.node_ids.include?(edge.to)

          line.points.each_cons(2) do |(x1, y1), (x2, y2)|
            distance = [(x2 - x1).abs, (y2 - y1).abs].max
            (0..distance).each do |step|
              x = x1 + (x2 <=> x1) * step
              y = y1 + (y2 <=> y1) * step
              expect(x <= frame.x || x >= frame.x + frame.width - 1 ||
                     y <= frame.y || y >= frame.y + frame.height - 1).to be(true)
            end
          end
        end
      end
      frames.combination(2).each do |a, b|
        disjoint = a.x + a.width <= b.x || b.x + b.width <= a.x ||
                   a.y + a.height <= b.y || b.y + b.height <= a.y
        contains = lambda do |outer, inner|
          outer.x <= inner.x && outer.y <= inner.y &&
            outer.x + outer.width >= inner.x + inner.width &&
            outer.y + outer.height >= inner.y + inner.height
        end
        expect(disjoint || contains.call(a, b) || contains.call(b, a)).to be(true)
      end
    end
  end

  it "clips group endpoint edges to the group frame" do
    source = File.read(File.expand_path("fixtures/subgraph_cases/group_10_endpoint.mmd", __dir__))
    scene = described_class.parse(source).scene
    frames = scene.items.grep(Merminal::Scene::Box).select { |item| item.role == :container_border }.map(&:rect)
    edges = scene.items.grep(Merminal::Scene::Polyline).select { |item| item.role == :edge }
    inner = frames.last
    expect(edges.first.points.last[1]).to eq(inner.y)
    expect(edges.last.points.first[0]).to eq(inner.x + inner.width - 1)
  end

  it "keeps long subgraph titles inside their frames" do
    source = "flowchart TB\nsubgraph G[An unusually long group title]\nA-->B\nend\n"
    document = described_class.parse(source)
    frame = document.scene.items.grep(Merminal::Scene::Box).find { |item| item.role == :container_border }.rect
    expect(frame.width).to be >= Merminal::Text.width("An unusually long group title") + 4
  end

  it "accepts arbitrary bytes without leaking encoding exceptions" do
    rng = Random.new(118)
    headers = %w[graph sequenceDiagram stateDiagram-v2 classDiagram erDiagram pie xychart-beta gantt timeline mindmap]
    headers.each do |header|
      20.times do
        bytes = Array.new(30) { rng.rand(256) }.pack("C*")
        expect { described_class.render("#{header}\n".b + bytes) }.not_to raise_error
      end
    end
  end

  it "survives truncated and mutated diagrams" do
    rng = Random.new(5_503)
    Dir.glob(File.expand_path("fixtures/*.mmd", __dir__)).each do |path|
      source = File.read(path)
      50.times do
        truncated = source.byteslice(0, rng.rand(source.bytesize))
        mutated = source.dup
        mutated[rng.rand(mutated.length)] = ["@", "}", "[", "\n"].sample(random: rng)
        [truncated, mutated].each do |input|
          begin
            described_class.render(input)
          rescue Merminal::UnsupportedDiagramError, Merminal::SyntaxError
            nil
          rescue RangeError => error
            raise "#{path}: #{input.inspect}: #{error.message}"
          end
        end
      end
    end
  end

  it "matches the reviewed Unicode gallery snapshots" do
    Dir.glob(File.expand_path("fixtures/**/*.mmd", __dir__)).each do |path|
      document = described_class.parse(File.read(path))
      expect(document.diagnostics.select { |diagnostic| diagnostic.severity == :error }).to be_empty
      expect(document.render).to match_snapshot(File.basename(path, ".mmd"))
    end
  end

  def assert_flowchart_properties(source)
    document = described_class.parse(source)
    return if document.ast.nodes.empty?

    scene = document.scene
    output = document.render
    expect(output.lines.map { |line| Merminal::Text.width(line.chomp) }.max).to be <= scene.width
    expect(output).to eq(document.render)
    boxes = scene.items.grep(Merminal::Scene::Box).select { |item| item.role == :node_border }.map(&:rect)
    frames = document.ast.subgraphs.map(&:id).zip(scene.items.grep(Merminal::Scene::Box)
                     .select { |item| item.role == :container_border }.map(&:rect)).to_h
    boxes.combination(2).each do |a, b|
      expect(a.x + a.width <= b.x || b.x + b.width <= a.x || a.y + a.height <= b.y || b.y + b.height <= a.y).to be(true)
    end
    lines = scene.items.grep(Merminal::Scene::Polyline).select { |item| item.role == :edge }
    document.ast.edges.reject { |edge| edge.stroke == :invisible }.zip(lines).each do |ast_edge, edge|
      group_ids = document.ast.styles["group_edge:#{ast_edge.id}"] || []
      endpoints = boxes + group_ids.filter_map { |id| frames[id] }
      [edge.points.first, edge.points.last].each do |x, y|
        expect(endpoints.any? do |box|
          x >= box.x && x < box.x + box.width && y >= box.y && y < box.y + box.height &&
            (x == box.x || x == box.x + box.width - 1 || y == box.y || y == box.y + box.height - 1)
        end).to be(true)
      end
      edge.points.each_cons(2) do |(x1, y1), (x2, y2)|
        distance = [(x2 - x1).abs, (y2 - y1).abs].max
        (0..distance).each do |step|
          x = x1 + (x2 <=> x1) * step
          y = y1 + (y2 <=> y1) * step
          expect(boxes.none? { |box| x > box.x && x < box.x + box.width - 1 && y > box.y && y < box.y + box.height - 1 }).to be(true)
        end
      end
    end
    labels = scene.items.grep(Merminal::Scene::Text).select { |item| item.role == :edge_label }
    labels.each do |label|
      right = label.x + Merminal::Text.width(label.string)
      expect(boxes.none? { |box| label.y >= box.y && label.y < box.y + box.height && right > box.x && label.x < box.x + box.width }).to be(true)
    end
    labels.combination(2).each do |a, b|
      next unless a.y == b.y

      expect(a.x + Merminal::Text.width(a.string) <= b.x ||
             b.x + Merminal::Text.width(b.string) <= a.x).to be(true)
    end
    document.ast.nodes.each { |node| expect(output).to include(node.label) }
    expect(document.render(charset: :ascii)).to match(/\A[\x20-\x7e\n]*\z/)
  end
end
